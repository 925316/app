    ...
    if (typeof window.BladewindDropmenu === "undefined") {
        window.BladewindDropmenu = class BladewindDropmenu {
            ...
            constructor(name, options = {}) {
                this.name = name;
                this.items = `.${name} .dropmenu-items`;
            ...
            show = () => {
                ...
                if (this.hasItems()) {
                    ...
                    if (
                        this.options.hideAfterClick &&
                        domEls(`${this.items} .item`)
                    ) {
                        domEls(`${this.items} .item`).forEach((item) => {
            ...
            toggle = () => {
                domEl(this.items).getAttribute("data-open") === "0"
                    ? this.show()
                    : this.hide();
            };

            activate = () => {
                domEl(`.${this.name} .trigger`).addEventListener(
