function initCalendar({
    ...
    datePicker.classList.add(`datepicker`, `${inputId}`, "hidden");
    ...
    function positionCalendar() {
        ...
        if (
            inputRect.bottom + datePickerRect.height > viewportHeight &&
            inputRect.top > datePickerRect.height
        ) {
    ...
    function renderDays() {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            if (
                (minDate && date.getTime() < minDate.setHours(0, 0, 0, 0)) ||
    ...
    function renderMonths() {
        ...
        container.className =
            "month-container grid grid-cols-3 divide-x divide-y border dark:border-dark-600/30 dark:divide-dark-600/30 dark:divide-dark-600/30 text-center -mx-2 -mb-2 radius-b-lg";
    ...
    function renderHeader(heading, isFullCalendar = false) {
        ...
        nextButton.innerHTML = domEl(".datepicker-right-arrow").innerHTML;
        prevButton.innerHTML = domEl(".datepicker-left-arrow").innerHTML;
        prevButton.classList.add("nav-button");
        nextButton.classList.add("nav-button");
        if (isFullCalendar) {
            let monthKey = currentDate
                .toLocaleString("en-US", { month: "short" })
                .toLowerCase();
            let thisMonth = MONTH_NAMES[monthKey] || monthKey;
            title.className =
                "month-year text-lg tracking-wider cursor-pointer font-light";
            title.innerHTML = `
            ...
            title.className =
                "calendar-title text-lg tracking-wider font-light";
    ...
    function renderYears() {
        ...
        const container = document.createElement("div");
        container.className =
            "year-container grid grid-cols-4 divide-x divide-y border dark:border-dark-600/30 dark:divide-dark-600/30 text-center -mx-2 -mb-2 radius-b-lg";
        ...
        for (let i = startYear - 1; i <= startYear + 10; i++) {
            ...
            if (
                (minDate && i < minDate.getFullYear()) ||
                (maxDate && i > maxDate.getFullYear())
            ) {
