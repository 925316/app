function addRemoveRowValue(value, name) {
    ...
    const table = domEl(`.table.${name}.selectable`);
    const checkAllBox = table.querySelector(
        'th:first-child input[type="checkbox"]',
    );
    const partiallyCheckedBox = table.querySelector(
        "th:first-child .check-icon",
    );
    ...
    if (value) {
        ...
        if (totalChecked > 0 && totalChecked < totalRows) {
            ...
            if (!partiallyCheckedBox.getAttribute("onclick")) {
                partiallyCheckedBox.setAttribute(
                    "onclick",
                    `checkAllFromPartiallyChecked('${name}')`,
                );
            }
        } else {
...
function checkAllFromPartiallyChecked(name) {
    ...
    toggleAll(checkAllBox, `.table.${name}`);
}

function listenToSelectableTableRowEvents(name) {
    domEls(`.table.${name}.selectable tr`).forEach((el) => {
...
function addCheckboxesToTable(el) {
    ...
    for (let row of table.rows) {
        let cellTag =
            row.parentElement.tagName.toLowerCase() === "thead" ? "th" : "td";
        let checkboxCell = document.createElement(cellTag);
        checkboxCell.innerHTML =
            cellTag === "th"
                ? checkboxHtml.replace(
                      'type="checkbox"',
                      `type="checkbox" onclick="toggleAll(this,'${el}')"`,
                  ) + partialCheckHtml
                : checkboxHtml;
...
function toggleAll(srcEl, table) {
    domEls(`${table}.selectable tr`).forEach((el) => {
        ...
        if (checkbox) {
            ...
            if (
                (srcEl.checked && !checkbox.checked) ||
                (!srcEl.checked && checkbox.checked)
            )
                el.click();
        }
...
function sortTableByColumn(el, table) {
    ...
    sortDirection =
        sortDirection === "no-sort"
            ...
            : sortDirection === "asc"
              ? "desc"
              : "no-sort";
    let sortColumn = domEl(
        `.${table} th[data-column-index="${sortColumnIndex}"]`,
    );
    ...
    if (sortDirection === "no-sort") {
        ...
        const rows = Array.from(tbody.rows).filter(
            (row) => row.getAttribute("data-page") === currentPage,
        );
        document.body.appendChild(tbody);

        rows.forEach((row) => {
            const cellValue = row.cells[sortColumnIndex].innerText.trim();
            row.sortKey = isNumeric(cellValue)
...
function resetToOriginalOrder(table, tbody, currentPage) {
    ...
    const currentPageRows = originalRows.filter(
        (row) => row.getAttribute("data-page") === currentPage,
    );
