    ...
    if (scope) {
        if (typeof scope === "string") {
            if (!scope.includes(".") && !scope.includes("#")) {
                console.log(
                    `${scope} needs to contain . or # to target a DOM element`,
                );
            }
...
const hide = (element, elementIsDomObject = false) => {
    if (
        (!elementIsDomObject && domEl(element) != null) ||
        (elementIsDomObject && element != null)
    ) {
...
const clearErrors = (obj) => {
    ...
    if (el.value !== "") {
        if (elParent !== null) {
            domEl(`.${elParent} .clickable`).classList.remove(
                "!border-red-400",
            );
        } else {
    ...
    try {
        ...
        fieldToValidate.forEach((el) => {
            ...
            if (isEmpty(el.value)) {
                ...
                el.addEventListener(
                    "keyup",
                    clearErrors.bind(null, listenerObj),
                    false,
                );
...
const isNumberKey = (event, with_dots = 1) => {
    ...
    if (
        !event.key.toString().match(acceptedKeys) &&
        event.key !== "Enter" &&
        event.key !== "Tab"
    ) {
...
const showModal = (element, placeholders = {}) => {
    unhide(`.${element}-modal`);
    document.body.classList.add("overflow-hidden");
    domEl(`.${element}-modal`).focus();
    let index = openModals.length === 0 ? 0 : openModals.length + 1;
    animateCss(`.${element}`, "zoomIn").then(() => {
        openModals[index] = element;
        if (Object.keys(placeholders).length > 0) {
            const modalBody = domEl(`.${element}-modal .modal-body`);
            if (!window.originalContent) {
                window.originalContent = modalBody.innerHTML;
            }
            modalBody.innerHTML = window.originalContent.replace(
                /:([\w]+)/g,
...
const trapFocusInModal = (event) => {
    ...
    if (modalName !== undefined) {
        const focusableElements = domEls(
            `.${modalName}-modal input:not([type='hidden']):not([class*='hidden']), .${modalName}-modal button:not([class*="hidden"]),  .${modalName}-modal a:not([class*="hidden"])`,
        );
...
const hideModal = (element) => {
    animateCss(`.${element}`, "zoomOut").then(() => {
        openModals.pop();
        document.body.classList.remove("overflow-hidden");
        domEl(`.${element}-modal`).removeEventListener(
            "keydown",
            trapFocusInModal,
        );
        animateCss(`.${element}-modal`, "zoomOut").then(() => {
            hide(`.${element}-modal`);
        });
    });
};
...
const goToTab = (element, colour, scope) => {
    ...
    let tabContent = domEl(".tc-" + element);
    if (tabContent === null) return false;

    changeCssForDomArray(
        `.${scope}-headings li.atab span`,
        `${colour}, is-active`,
        "remove",
    );
...
const positionPrefix = (element, mode = "blur") => {
    let transparency = domEl(`.dv-${element} .prefix`).getAttribute(
        "data-transparency",
    );
    let offset = transparency === "1" ? -5 : 7;
    let prefixWidth =
        (getPrefixSuffixOffsetWidth(`.dv-${element} .prefix`) + offset) * 1;
...
const positionSuffix = (element) => {
    let transparency = domEl(`.dv-${element} .suffix`).getAttribute(
        "data-transparency",
    );
    let offset = transparency === "1" ? -5 : 7;
    let suffixWidth =
        (getPrefixSuffixOffsetWidth(`.dv-${element} .suffix`) + offset) * 1;
...
const filterTable = (keyword, table, field, tableData) => {
    ...
    hideList.forEach((row) => {
        let thisRow =
            currentPage !== null
                ? `${table} tbody tr[data-id="${row.id}"][data-page="${currentPage}"]`
                : `${table} tbody tr[data-id="${row.id}"]`;
...
const filterTableDebounced = (
    ...
    let rows =
        currentPage !== null
            ? `${table} tbody tr.hidden[data-page="${currentPage}"]`
            : `${table} tbody tr.hidden`;
    if (keyword.length >= minLength) {
        return (...args) => {
            clearTimeout(debounceTimerId);
            debounceTimerId = setTimeout(
                () => filterTable(keyword, table, field, tableData),
                delay,
            );
        };
    } else {
...
const selectTag = (value, name) => {
    ...
    if (input.value.includes(value)) {
        ...
        changeCss(
            tag,
            css.match(/bg-[\w]+-500/)[0].replace("500", "200/80"),
            "add",
            true,
        );
        ...
        changeCss(
            tag,
            css.match(/text-[\w]+-50/)[0].replace("50", "600"),
            "add",
            true,
        );
    } else {
        // add
        let total_selected =
            input.value === "" ? 0 : input.value.split(",").length;
        if (total_selected < max_selection) {
            input.value += `,${value}`;
            changeCss(tag, css.match(/bg-[\w]+-200\/80/)[0], "remove", true);
            changeCss(
                tag,
                css.match(/bg-[\w]+-200\/80/)[0].replace("200/80", "500"),
                "add",
                true,
            );
            ...
            changeCss(
                tag,
                css.match(/text-[\w]+-600/)[0].replace("600", "50"),
                "add",
                true,
            );
        } else {
            showNotification(
                input.getAttribute("data-error-heading"),
        ...
        if (startDate !== "" && endDate !== "") {
            if (startDate > endDate) {
                ...
                inline !== 1
                    ...
                    : (domEl(`.error-${element1}${element2}`).innerHTML =
                          message);
                return false;
            } else {
...
const checkMinMax = (min, max, element, enforceLimits = false) => {
    ...
    if (field.value !== "") {
        if (enforceLimits) {
            ...
            if (
                (!isNaN(minimum) && field.value < minimum) ||
                (!isNaN(maximum) && field.value > maximum)
            ) {
                changeCss(
                    field,
                    "focus:outline-primary-500,focus:border-primary-500",
...
const makeClearable = (element) => {
    ...
    let tableElement = element.replace("search_", "table.");
    let clearingFunction = domEl(tableElement)
        ? field.getAttribute("oninput").replace("this.value", "''")
        : "";
    if (!suffixElement.getAttribute("onclick")) {
        suffixElement.setAttribute(
            "onclick",
            `domEl(\'.${element}\').value=''; hide(this, true); ${clearingFunction}`,
        );
    }
    field.value !== ""
        ? unhide(suffixElement, true)
        : hide(suffixElement, true);
};
