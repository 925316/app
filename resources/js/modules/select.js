    ...
    if (typeof window.Select === "undefined") {
        window.Select = class Select {
            ...
            constructor(name, placeholder) {
                ...
                this.itemsContainer = `${this.rootElement} .select-items-container`;
                this.searchInput = `${this.itemsContainer} .search`;
                this.selectItems = this.strSelectItems();
                this.isMultiple =
                    domEl(this.rootElement).getAttribute("data-multiple") ===
                    "true";
                this.required =
                    domEl(this.rootElement).getAttribute("data-required") ===
                    "true";
                this.formInput = `input.${this.name}`;
                domEl(this.displayArea).style.maxWidth =
                    `${domEl(this.rootElement).offsetWidth - 40}px`;
                this.maxSelection = -1;
                this.canClear = !this.required && !this.isMultiple;
                this.enabled = true;
                this.selectedItem = null;
                this.metaData =
                    domEl(this.rootElement).getAttribute("data-meta-data") ||
                    null;
            }
                ...
                domEl(this.rootElement).addEventListener("keydown", (e) => {
                    ...
                    if (e.key === "ArrowDown" || e.key === "ArrowUp") {
                        ...
                        let els = [...domEls(this.selectItems)].filter((el) => {
                            ...
                            return (
                                el.getAttribute("data-unselectable") === null
                            );
                        });
            ...
            hide = () => {
                document.addEventListener("mouseup", (e) => {
                    ...
                    let container = domEl(
                        this.isMultiple ? this.itemsContainer : this.clickArea,
                    );
                    if (
                        searchArea &&
                        container &&
            ...
            search = () => {
                domEl(this.searchInput).addEventListener("keyup", (e) => {
                    let searchScope =
                        domEl(this.rootElement).getAttribute(
                            "data-filter-by",
                        ) || null;
                    let value = domEl(this.searchInput).value;
                    let items = searchScope
                        ? `${this.selectItems}[data-filter-value="${searchScope}"]`
                        : this.selectItems;
            ...
            manualModePreSelection = () => {
                let selectMode = domEl(`${this.rootElement}`).getAttribute(
                    "data-type",
                );
                let selectedValue = domEl(`${this.rootElement}`).getAttribute(
                    "data-selected-value",
                );
                if (selectMode === "manual" && selectedValue !== null) {
                    domEls(this.selectItems).forEach((el) => {
                        let item_value = el.getAttribute("data-value");
                        if (item_value === selectedValue)
            ...
            setEmptyStateMessage = (element = this.name) => {
                ...
                const copyFrom = root?.getAttribute(
                    "data-copy-empty-state-from",
                );
                const source = domEl(`.empty-state.${copyFrom}`);
                const target = domEl(
                    this.strSelectItems(element, " div.empty-state-copy"),
                );
            ...
            items = (element = this.name) => {
                return domEls(
                    this.strSelectItems(
                        element,
                        `:not(.hidden):not(.empty-state)`,
                    ),
                );
            };

            showHideEmptyState = (element = this.name) => {
                const emptyStateItem = domEl(
                    this.strSelectItems(element, ".empty-state"),
                );
                ...
                hasNoItems
                    ? unhide(emptyStateItem, true)
                    : hide(emptyStateItem, true);
            ...
            moveLabel = (direction = "up") => {
                let placeholderElement = domEl(
                    `${this.rootElement} .placeholder`,
                );
                let labelElement = domEl(
                    `${this.rootElement} .placeholder .form-label`,
                );
                if (labelElement) {
                    if (direction === "up") {
            ...
            setValue = (item) => {
                this.selectedValue = item
                    ...
                    : null;
                let selectedLabel = item
                    ? item.getAttribute("data-label")
                    : null;
                let svg = domEl(
                    `${this.rootElement} div[data-value="${this.selectedValue}"] svg`,
                );
                ...
                if (this.enabled) {
                    if (!this.isMultiple) {
                        ...
                        if (this.canClear) {
                            ...
                            domEl(`${this.clickArea} .reset`).addEventListener(
                                "click",
                                (e) => {
                        ...
                        if (input.value.includes(this.selectedValue)) {
                            ...
                            if (!this.maxSelectableExceeded()) {
                                unhide(svg, true);
                                input.value += `,${this.selectedValue}`;
                                domEl(this.displayArea).innerHTML +=
                                    this.labelTemplate(
                                        selectedLabel,
                                        this.selectedValue,
                                    );
            ...
            unsetValue = (item) => {
                ...
                if (this.enabled) {
                    //!domEl(this.clickArea).classList.contains('disabled')
                    if (!this.isMultiple) {
                        ...
                        if (
                            domEl(
                                `${this.displayArea} span.sp-${this.selectedValue}`,
                            )
                        ) {
                            let keyword = `(,?)${this.selectedValue}`;
                            input.value = input.value.replace(
                                input.value.match(keyword)[0],
                                "",
                            );
                            hide(svg, true);
                            domEl(
                    ...
                    if (this.toFilter) {
                        new Select(this.toFilter, "").reset(); //FIXME: dont new up an instance
            ...
            scrollbars = () => {
                if (
                    domEl(this.displayArea).scrollWidth >
                    domEl(this.rootElement).clientWidth
                ) {
                    unhide(`${this.clickArea} .scroll-left`);
                    unhide(`${this.clickArea} .scroll-right`);
                    domEl(`${this.clickArea} .scroll-right`).addEventListener(
                        "click",
                        (e) => {
                            this.scroll(150);
                            e.stopImmediatePropagation();
                    ...
                    domEl(`${this.clickArea} .scroll-left`).addEventListener(
                        "click",
                        (e) => {
                            this.scroll(-150);
                            e.stopImmediatePropagation();
            ...
            scroll = (amount) => {
                ...
                domEl(this.displayArea).clientWidth +
                    domEl(this.displayArea).scrollLeft >=
                domEl(this.displayArea).scrollWidth
                    ? hide(`${this.clickArea} .scroll-right`)
                    : unhide(`${this.clickArea} .scroll-right`);
            ...
            labelTemplate = (label, value) => {
                return (
                    `<span class="bg-slate-200 hover:bg-slate-300 dark:bg-dark-800 dark:hover:bg-dark-900 
                    inline-flex items-center text-slate-700 dark:text-dark-300 py-[2.5px] pl-2.5 pr-1 ` +
                    `mr-2 text-sm rounded-md sp-${value} animate__animated animate__bounceIn animate__faster" ` +
            ...
            removeLabel = () => {
                domEls(`${this.displayArea} span svg`).forEach((el) => {
                    el.addEventListener("click", (e) => {
                        ...
                        this.unsetValue(
                            domEl(`.select-item[data-value="${value}"]`),
                        );
                    });
                });
            };
            ...
            callUserFunction = (item) => {
                let userFunction = item
                    ? item.getAttribute("data-user-function")
                    : null;
            ...
            maxSelectableExceeded = () => {
                ...
                return (
                    this.maxSelection !== -1 &&
                    totalSelected === this.maxSelection
                );
            };

            strSelectItems = (name = this.name, options = "") => {
                return `.select-${name} .select-items .select-item${options}`;
            };

            strRootElement = (name = this.name) => {
                return name.includes("select") ? name : `.select-${name}`;
            };

            isSearchable = (name = this.name) => {
                return domEl(this.strRootElement(name)).className.includes(
                    "searchable",
                );
            };
            ...
            filter = (element, by = "", skipReset = true) => {
                ...
                if (by !== "") {
                    ...
                    domEl(`.select-${this.toFilter}`).setAttribute(
                        "data-filter-by",
                        by,
                    );
            ...
            addItem = (value, label) => {
                const container = domEl(`${this.rootElement} .select-items`);
                if (!container) return;
            ...
            removeItem = ({ value = null, index = null }) => {
                ...
                if (value !== null) {
                    item = domEl(
                        `${this.rootElement} .select-item[data-value="${value}"]`,
                    );
                } else if (index !== null) {
                    const items = domEls(`${this.rootElement} .select-item`);
                    item = items[index];
                }
                if (item) item.remove();
            };
