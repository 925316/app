            ...
            <div class="mb-4">
                ...
                <select name="used_by" id="used_by"
                    ...
                    @foreach($accounts as $account)
                        <option
                            value="{{ $account->id }}" {{ old('used_by', $license->used_by) == $account->id ? 'selected' : '' }}>
            ...
            <div class="mb-4">
                ...
                <select name="privilege" id="privilege"
                    ...
                    @foreach($privilegeOptions as $value => $label)
                        <option
                            value="{{ $value }}" {{ old('privilege', $license->privilege->value) == $value ? 'selected' : '' }}>
                            {{ ucfirst($label) }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <select name="status" id="status"
                    ...
                    @foreach($statusOptions as $value => $label)
                        <option
                            value="{{ $value }}" {{ old('status', $license->status->value) == $value ? 'selected' : '' }}>
                            {{ ucfirst($label) }}
                        </option>
                    @endforeach
                </select>
