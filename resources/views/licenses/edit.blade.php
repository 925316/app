            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('licenses.update', $license) }}">
                        ...
                        <div class="mb-4">
                            <label for="key" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">License Key</label>
                        ...
                        <!-- Account -->
                        <div class="mb-4">
                            <label for="used_by" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Assign to Account</label>
                            <select name="used_by" id="used_by" class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300"
                                    {{ $license->status !== \App\Enums\LicenseStatus::UNUSED ? 'disabled' : '' }}>
                                <option value="">Unassigned</option>
                                @foreach($accounts as $account)
                                    <option value="{{ $account->id }}" {{ old('used_by', $license->used_by) == $account->id ? 'selected' : '' }}>
                                        {{ $account->username }} ({{ $account->email }})
                                    </option>
                                @endforeach
                            </select>
                            @if($license->status !== \App\Enums\LicenseStatus::UNUSED)
                        ...
                        <div class="mb-4">
                            ...
                            <select name="status" id="status" class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300">
                                @foreach($statusOptions as $value => $label)
                                    <option value="{{ $value }}" {{ old('status', $license->status->value) == $value ? 'selected' : '' }}>
                                        {{ ucfirst($label) }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="flex justify-end gap-3 mt-6">
                            <a href="{{ route('licenses.show', $license) }}" class="px-4 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-700 transition">
                                Cancel
                            </a>
