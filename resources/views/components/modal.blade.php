...
@props([
    ...
    'name' => defaultBladewindName('modal-'),
...
<div data-name="{{$name}}" data-backdrop-can-close="{{$backdropCanClose}}"
     class="fixed inset-0 flex items-center justify-center bg-black/40 {{$blur_intensity()}} z-40 flex modal {{$name}}-modal hidden overscroll-contain">
    <div class="{{$sizes[$size]}} @if($size=='omg') sm:px-12 @else max-w-screen @endif px-5 m-auto {{$name}} animate__faster">
                ...
                @if(!empty($type) || !empty($icon))
                    <div class="modal-icon grow-0 pr-2 pt-1.5">
                        ...
                        @if(!empty($icon) && empty($type))
                            <x-icon name="{{ $icon }}" class="size-14 {{$iconCss}}"/>
                        @endif
                    </div>
                @endif
            ...
            @if( $showActionButtons )
                <div @class([
                    ...
                    <x-button
                            ...
                            @class([
                                'cancel ' . $cancelCss,
                                'block w-full mt-2' => $stretchActionButtons])>{{$cancelButtonLabel}}</x-button>
                    ...
                    <x-button
                            ...
                            @class([
                                'okay ' . $okCss,
                                'block w-full' => $stretchActionButtons])>{{$okButtonLabel}}</x-button>
                </div>
            @endif
        </div>
    </div>
...
<x-script :nonce="$nonce">
    domEl('.{{$name}}-modal').addEventListener('click', function (e) {
    let backdropCanClose = this.getAttribute('data-backdrop-can-close');
    if (backdropCanClose) hideModal('{{$name}}');
    });

    domEl('.{{$name}}').addEventListener('click', function (e) {
    e.stopImmediatePropagation();
    });

    if (domEls('.{{$name}}-modal .modal-footer>button')) {
    domEls('.{{$name}}-modal .modal-footer>button').forEach((el) => {
    ...
    if (domEl(`.${modalName}-modal`).getAttribute('data-backdrop-can-close') === '1') {
    hideModal(modalName);
    e.stopImmediatePropagation();
    }
    }
