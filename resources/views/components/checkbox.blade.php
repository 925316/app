...
<label class="inline-flex items-center cursor-pointer text-sm @if($disabled) opacity-60 @endif @if($addClearing) mb-3 @endif {{ $labelCss }}">
    <input
            type="{{ $type }}"
            name="{{ $name }}"
            class="{{$text_colour}} size-6 @if($addClearing) mr-2 rtl:ml-2 @endif disabled:opacity-50 focus:{{$ring_colour}} border-2 {{$border_colour}} focus:ring-opacity-25 checkbox {{$class}}"
