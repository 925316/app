{{-- format-ignore-start --}}
@props([
    'name' => defaultBladewindName('slider-'),
    'min' => 0,
    'max' => 100,
    'step' => 1,
    'selected' => 0,
...
<div class="slider-container w-full relative {{$class}}">
    <input type="range"
           min="{{$min}}"
           max="{{$max}}"
           value="{{$selected}}"
           step="{{$step}}"
           class="slider min-slider-{{$name}} {{$colour}}"/>
    @if($range)
        <input type="range"
               min="{{$min}}"
               max="{{$max}}"
               value="{{$maxSelected}}"
               step="{{$step}}"
               class="slider max-slider-{{$name}} {{$colour}}"/>
    @endif
    ...
    <input type="hidden"
           name="{{$name}}"
           id="{{$name}}"
           class="slider-selection-{{$name}}-input slider-{{$name}}"
