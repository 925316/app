...
@endphp
{{-- format-ignore-end --}}
<div class="fixed flex flex-col-reverse {{ $position_css[str_replace('-', '_', $position)] }} z-50 notification-container w-11/12"></div>
{{--sm:w-1/4 sm:w-96 md:w-96  w-11/12--}}
<div class="notification-icons hidden">
    <x-modal-icon class="hidden {{$css}}"/>
    <x-modal-icon class="hidden {{$css}}" type="error"/>
    <x-modal-icon class="hidden {{$css}}" type="warning"/>
    <x-modal-icon class="hidden {{$css}}" type="success"/>
</div>

<x-script :nonce="$nonce">
    @php include_once(public_path('resources/js/modules/notification.js')); @endphp
</x-script>
