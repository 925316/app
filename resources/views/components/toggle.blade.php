...
@props([
    ...
    'name' => defaultBladewindName('toggle-'),
...
<label class="relative @if(!$justified)inline-flex @else flex justify-between @endif items-center group tgl-{{$name}}">
    ...
    after:rounded-full tgl-sp-{{$name}} {{$bar_circle_size[$bar]}} {{$bar_colour}} {{$class}}"></span>
