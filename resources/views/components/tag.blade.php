...
@php
    ...
    if( (!empty($name) && !empty($value)) ) {
        ...
        $bg_border_colour_css = "bg-$color-200/80 hover:bg-$color-600 cursor-pointer selectable $name-$value";
...
<label id="@if($addIdPrefix)@endif{{$id}}" @if($selectable) onclick="selectTag('{{$value}}','{{$name}}')" @endif
