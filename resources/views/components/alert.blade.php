...
<div class="w-full alert animate__animated animate__fadeIn rounded-md flex p-3  {{$colours[$shade] }} {{ $class }}">
    @if($showIcon)
        <div class="pt-[1px]">
            @if($icon !== '')
                <x-icon :name="$icon" class="-mt-1 {{ $iconAvatarCss}}"/>
            @elseif($avatar !== '')
                <x-avatar :image="$avatar" :show_ring="$showRing" :size="$size"
                                     class="{{ $iconAvatarCss}}"/>
            @else
                <x-modal-icon type="{{$type}}"
