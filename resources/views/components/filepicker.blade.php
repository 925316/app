...
@props([
    ...
    // this is prefixed with  when used as a class name
    'name' => defaultBladewindName('file-'),
...
@php
    ...
    if(!app()->environment('production') && !file_exists('resources/css/modules/filepond.min.css')) {
...
@endphp
@once
    <link href="{{ asset('resources/css/modules/filepond.css') }}" rel="stylesheet"/>
    @if($hasImageFiles)
    <link href="{{ asset('resources/css/modules/filepond-plugin-image-preview.css') }}" rel="stylesheet"/>
    <x-script :nonce="$nonce" src="{{ asset('resources/js/modules/filepond-plugin-image-exif-orientation.js') }}"></x-script>
    <link href="{{ asset('resources/css/modules/cropper.min.css') }}" rel="stylesheet">
    <x-script :nonce="$nonce" src="{{ asset('resources/js/modules/cropper.min.js') }}"></x-script>
    <x-script :nonce="$nonce" src="{{ asset('resources/js/modules/filepond-plugin-image-crop.js') }}"></x-script>
    <x-script :nonce="$nonce" src="{{ asset('resources/js/modules/filepond-plugin-image-preview.js') }}"></x-script>
    <x-script :nonce="$nonce" src="{{ asset('resources/js/modules/filepond-plugin-image-resize.js') }}"></x-script>
    <x-script :nonce="$nonce" src="{{ asset('resources/js/modules/filepond-plugin-image-transform.js') }}"></x-script>
    @endif
    @if($validateFileSize)
        <x-script :nonce="$nonce" src="{{ asset('resources/js/modules/filepond-plugin-file-validate-size.js') }}"></x-script>
        @if($hasImageFiles)<x-script :nonce="$nonce" src="{{ asset('resources/js/modules/filepond-plugin-image-validate-size.js') }}"></x-script>@endif
    @endif
    <x-script :nonce="$nonce" src="{{ asset('resources/js/modules/filepond-plugin-file-encode.js') }}"></x-script>
    <x-script :nonce="$nonce" src="{{ asset('resources/js/modules/filepond-plugin-file-validate-type.js') }}"></x-script>
    <x-script :nonce="$nonce" src="{{ asset('resources/js/modules/filepond.min.js') }}"></x-script>
    <x-script :nonce="$nonce">const csrfToken = document.querySelector('meta[name="csrf-token"]')?.getAttribute("content") || "{{ csrf_token() }}";</x-script>
@endonce
{{-- format-ignore-end --}}

@if($canCrop)
    @once
        <x-modal name="cropper-container" size="large" align_buttons="center" backdrop_can_close="false" ok_button_label="Crop">
            <img class="cropper-image max-w-full" />
        </x-modal>
    @endonce
@endif

<div class="filepicker-placeholder-{{$cleanName}} space-y-2 flex hidden align-middle py-3">
    <div>
        <x-icon name="note-2" class="size-14 rounded-full p-2 bg-gray-200/50 stroke-2 text-slate-400 dark:bg-dark-100/50 dark:invert dark:opacity-30"/>
    </div>
    <div class="text-left pl-2.5 pt-1.5 dark:text-dark-200">
        <div>{!! $placeholderLine1 !!} @if($required)<x-icon name="star" class="!text-red-400 !w-2 !h-2 mt-[-2px]" type="solid"/>@endif</div>
...
@if($canCrop) @once
    ...
    const cropButton = domEl('.cropper-container-modal .okay');
    let cropper;
@endonce @endif
...
const pond_{{$cleanName}} = FilePond.create(domEl('input[name="{{$name}}"]'), {
        ...
        labelIdle: (domEl(".placeholder-{{$cleanName}}")) ? domEl(".placeholder-{{$cleanName}}").innerHTML : domEl(".filepicker-placeholder-{{$cleanName}}").innerHTML,
        acceptedFileTypes: {!! json_encode(explode(',', str_replace(' ', '', $acceptedFileTypes))) !!},
        labelInvalidField: '{{ __("bladewind.filepicker_invalid_field")}}',
        labelFileWaitingForSize: '{{ __("bladewind.filepicker_waiting_for_size")}}',
        labelFileSizeNotAvailable: '{{ __("bladewind.filepicker_file_size_not_available")}}',
        labelFileLoading: '{{ __("bladewind.filepicker_file_loading")}}',
        labelFileLoadError: '{{ __("bladewind.filepicker_file_load_error")}}',
        labelFileProcessing: '{{ __("bladewind.filepicker_file_processing")}}',
        labelFileProcessingComplete: '{{ __("bladewind.filepicker_file_processing_complete")}}',
        labelFileProcessingAborted: '{{ __("bladewind.filepicker_file_processing_aborted")}}',
        labelFileProcessingError: '{{ __("bladewind.filepicker_file_processing_error")}}',
        labelFileProcessingRevertError: '{{ __("bladewind.filepicker_file_processing_revert_error")}}',
        labelFileRemoveError: '{{ __("bladewind.filepicker_file_remove_error")}}',
        labelTapToCancel: '{{ __("bladewind.cancel")}}',
        labelTapToRetry: '{{ __("bladewind.retry")}}',
        labelTapToUndo: '{{ __("bladewind.undo")}}',
        labelButtonRemoveItem: '{{ __("bladewind.remove")}}',
        labelButtonAbortItemLoad: '{{ __("bladewind.abort")}}',
        labelButtonRetryItemLoad: '{{ __("bladewind.retry")}}',
        labelButtonAbortItemProcessing: '{{ __("bladewind.cancel")}}',
        labelButtonUndoItemProcessing: '{{ __("bladewind.undo")}}',
        labelButtonRetryItemProcessing: '{{ __("bladewind.retry")}}',
        labelButtonProcessItem: '{{ __("bladewind.upload")}}',
        labelMaxFileSizeExceeded: '{{ __("bladewind.filepicker_max_file_size_exceeded")}}',
        labelMaxTotalFileSizeExceeded: '{{ __("bladewind.filepicker_max_total_file_size_exceeded")}}',
        labelMaxFileSize: '{{__("bladewind.filepicker_max_file_size")}}',
        labelMaxTotalFileSize: '{{__("bladewind.filepicker_max_total_file_size")}}',
        labelFileTypeNotAllowed: '{{__("bladewind.filepicker_invalid_file_type")}}',
        fileValidateTypeLabelExpectedTypes: '{{__("bladewind.filepicker_expected_file_types")}}',
        ...
        iconRemove: `<x-icon name="trash" class="p-1 opacity-80 hover:text-red-500" type="solid" />`,
