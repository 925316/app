...
@props([
    ...
    */
    ...
    'errorMessage' => __('bladewind.otp_error_message'),
...
<div class="dv-{{ $name }} relative">
    ...
    <div class="text-center text-sm text-error-500 my-6 hidden {{ $name }}-pin-error">
        {!! $errorMessage !!}
    </div>
    <div class="text-center tracking-wider text-sm my-6 {{ $name }}-pin-timer">
        <div class="countdown hidden"><span class="minutes"></span>:<span class="seconds"></span></div>
        <div class="done"></div>
    </div>
    <div class="bg-transparent hidden absolute w-full z-40 flex items-center justify-center top-0 {{$cloak_size}} {{ $name }}-pin-spinner">
        <x-spinner/>
    </div>
    <div class="bg-transparent flex items-center justify-center w-full text-center hidden absolute top-0 z-40 {{$cloak_size}} {{ $name }}-pin-valid">
        <x-icon name="check-circle" type="solid" class="!size-9 text-green-500 mx-auto"/>
    </div>
    <div class="bg-transparent hidden w-full absolute top-0 {{$cloak_size}} {{ $name }}-pin-cloak"></div>
</div>
...
<x-script :nonce="$nonce">
    ...
    hide(`.${name}-pin-valid`);
    unhide(`.${name}-pin-spinner`);
    domEl(`.${name}-pcode0`).focus();
    domEl(`.${name}-pcode0`).blur();
    }

    var hideSpinner = (name) => {
    hide(`.${name}-pin-spinner`);
    }

    var showPinSuccess = (name) => {
    hide(`.${name}-pin-spinner`);
    unhide(`.${name}-pin-valid`);
    loseFocus(name);
    }
    ...
    let countdown_div = domEl(`.${name}-pin-timer .countdown`);
    let minutes_span = domEl(`.${name}-pin-timer .minutes`);
    let seconds_span = domEl(`.${name}-pin-timer .seconds`);
    let done_div = domEl(`.${name}-pin-timer .done`);
    unhide(`.${name}-pin-timer .countdown`);
    disablePin(name);
    countdown({
    name: name,
    minutes: minutes_,
    ...
    if (domEl('.code-timer-done')) {
    hide(options.countdownDiv, true);
    options.doneDiv.innerHTML = domEl('.code-timer-done').innerHTML;
    enablePin(name);
    }
    }
    }
