...
<div
        @class([
        "py-2 pl-4 pr-3 flex items-center text-base cursor-pointer select-item",
    ...
    @else
        ...
        @endif
        @if ($image !== '')
            <x-avatar size="tiny" class="!mt-0 !mr-2.5" image="{{ $image }}"/>
        @endif
        <span class="grow text-left">{!! $label !!}</span>
        <x-icon name="check-circle" class="text-slate-400 size-5 hidden svg-{{$value }}"/>
    @endif
</div>
