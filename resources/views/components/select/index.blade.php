...
@props([
    ...
    'name' => defaultBladewindName('select-'),

    // the default text to display when the select shows
    'placeholder' => config('bladewind.select.placeholder', __("bladewind.select_placeholder")),
    'searchPlaceholder' => config('bladewind.select.search_placeholder', __("bladewind.select_search_placeholder")),
    'emptyPlaceholder' => config('bladewind.select.empty_placeholder', __("bladewind.select_empty_placeholder")),
 ...
 * */
    ...
    */
    ...
    'maxErrorMessage' => config('bladewind.select.max_error_message', __("bladewind.select_max_selection")),
...
@php
    ...
    if($maxErrorMessage == '') $maxErrorMessage = __("bladewind.select_max_selection");
...
<div class="relative select select-{{$input_name}} @if($addClearing) mb-3 @endif @if($searchable) searchable @endif"
     role="combobox"
    ...
    <div tabindex="0"
        ...
        <div class="whitespace-nowrap inline-flex">
            <x-icon name="chevron-right" class="scroll-right !-mr-2 !mt-0.5 !w-5 !h-5 hidden"/>
            <x-icon
            ...
            <x-icon name="chevron-up-down" class="opacity-40 opener !ml-2"/>
        </div>
    </div>
    <div class="w-full absolute z-30 rounded-br-lg rounded-bl-lg bg-white shadow-sm shadow-gray-400 dark:shadow-none border-2
        ...
        hidden select-items-container overflow-scroll max-h-64 animate__animated animate__fadeIn animate__faster">
        <div class="sticky top-0 min-w-full bg-gray-100 dark:bg-transparent py-1 pr-0 -pl-1 search-bar @if(!$searchable) hidden @endif">
            <x-input
                    class="!border-0 !rounded-none focus:!outline-none !w-full !text-sm search"
                    add_clearing="false"
                    :placeholder="$searchPlaceholder"
                    suffix="magnifying-glass"
                    onfocus="changeCss('.select-{{$input_name}} .clickable', '!border-2, !outline-2, !-outline-offset-1, !outline-primary-500, !border-primary-500, dark:!border-primary-500, dark:!outline-primary-500')"
                    onblur="changeCss('.select-{{$input_name}} .clickable', '!border-2, !outline-2, !-outline-offset-1, !outline-primary-500, !border-primary-500, dark:!border-primary-500, dark:!outline-primary-500','remove')"
                    suffixIsIcon="true"/>
        </div>
        <div class="divide-y divide-gray-100 dark:divide-dark-600/80 select-items mt-0">
            @if($data !== 'manual')
    ...
    <input type="hidden" name="{{ ($dataSerializeAs !== '') ? $dataSerializeAs : $input_name }}"
           class="{{$input_name}} @if($required) required @endif"
           @if($required) data-parent="select-{{$input_name}}" @endif
           @if($multiple) autocomplete="off" @endif />
</div>
@once
    <x-script :nonce="$nonce" :src="asset('resources/js/modules/select.js')"></x-script>
@endonce
<x-script :nonce="$nonce" :modular="$modular">
    const {{ $input_name }} = new BladewindSelect('{{ $input_name }}', '{{ $placeholder }}');
    {{ $input_name }}.activate({disabled: '{{$disabled}}', readonly: '{{$readonly}}'});
    @if(!$disabled && !$readonly)
        {{ $input_name }}.maxSelectable({{$maxSelectable}}, '{{ sprintf($maxErrorMessage, $maxSelectable) }}');
    @endif
    @if(!empty($filter))
        {{ $input_name }}.filter('{{ $filter }}', '', false);
    @endif
    @if(!$required && $multiple == 'false')
        {{ $input_name }}.clearable();
    @endif
</x-script>