...
<div class="w-full accordions {{$name}} {{ $class }}"
     data-name="{{$name}}"
...
@once
    <x-script :nonce="$nonce">
        ...
        const parentAccordion = accordion.closest('.accordions');
        const canOpenMultiple = parentAccordion?.getAttribute('data-open-multiple') === '1';
        const accordions = domEls('.accordion', `.${parentAccordion?.getAttribute('data-name') || ''}`);
