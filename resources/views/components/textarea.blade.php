...
@props([
    ...
    'errorHeading' => config('bladewind.textarea.error_heading', __("bladewind.error_heading")),
...
<div class="relative w-full @if($addClearing) mb-3 @endif">
    ...
    @else
        <textarea
                {{ $attributes->merge(['class' => "input peer $is_required $name $placeholder_color focus:border-primary-500"]) }}
                id="{{ $name }}"
                name="{{ $name }}"
                rows="{{ $rows }}"
                @if($errorMessage !== '')
...
@if($toolbar)
    @once
        ...
        <x-script :nonce="$nonce"
                             src="https://cdn.jsdelivr.net/npm/quill@2.0.2/dist/quill.js"></x-script>
