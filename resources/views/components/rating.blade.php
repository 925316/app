...
@if($clickable)
    <x-input type="hidden" name="{{$name}}" id="{{$name}}" class="rating-value-{{$name}}"
                        selected_value="{{$rating}}"/>
@endif
<div class="h-{{$sizing[$size]+$size_adjustment}} overflow-hidden inline-block">
    @for ($x = 1; $x < 6; $x++)
        <div data-rating="{{$x}}"
             class="inline rating-{{$x}} {{$name}}@if($rating!= 0 && $x <= $rating*1) rated @endif"
...
<x-script :nonce="$nonce">
    ...
    if (domEl(`.rating-${y}.${name}`)) {
    if (!domEl(`.rating-${y}.${name}`).classList.contains('rated')) {
    if (mode == 'on') {
    hide(`.rating-${y}.${name} .empty`);
    unhide(`.rating-${y}.${name} .filled`);
    } else {
    unhide(`.rating-${y}.${name} .empty`);
    hide(`.rating-${y}.${name} .filled`);
    }
    }
    }
    }
    ...
    if (rate < 5) {
    for (x = rate; x <= 6; x++) {
    unhide(`.rating-${x}.${name} .empty`);
    hide(`.rating-${x}.${name} .filled`);
    }
    }
    for (x = 1; x <= rate; x++) {
    unhide(`.rating-${x}.${name} .filled`);
    hide(`.rating-${x}.${name} .empty`);
    changeCss(`.rating-${x}.${name}`, 'rated');
    }
