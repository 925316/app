...
@props([
    ...
    'defaultPage' => 1,
    'table' => null,
    'label' => __("bladewind.pagination_label"),
...
@if(!empty($totalRecords) && !empty($table))
    ...
    <div class="flex px-5 py-2 justify-between text-sm items-center opacity-80 pagination-{{$table}}">
        <div>
            @if($showTotal)
        ...
        <div>
            ...
            @elseif($style == 'dropdown')
                <div class="!z-50">
                    ...
                    <x-select :selected_value="$defaultPage" :required="true" data="manual"
                                         meta="{table: '{{$table}}'}" size="small">
                        @for($p=1; $p <= $total_pages; $p++)
                            <x-select.item :label="$p" :value="$p" onselect="routeToPage"/>
                        @endfor
                    </x-select>
                </div>
            @else
                ...
                <div class="flex">
                    ...
                    @for($p=1; $p <= $total_pages; $p++)
                        ...
                        <x-button
                                ...
                                onclick="routeToPage('{{$p}}', '', '', {table: '{{$table}}'}); shufflePageNumbers('{{$p}}', '{{$table}}')">{{$p}}</x-button>
                ...
                <x-script :nonce="$nonce">shufflePageNumbers('{{$defaultPage}}', '{{$table}}')
                </x-script>
            @endif
        </div>
    </div>
@endif
@once
    <span class="dots hidden"><x-icon name="ellipsis-horizontal"/></span>
    <x-script :nonce="$nonce">
        var selectPage = (page, previousPage, table) => {
        let pageButton = domEl(`.pagination-${table} button[data-page="${page}"]`);
        let prevPageButton = domEl(`.pagination-${table} button[data-page="${previousPage}"]`);
        ...
        let nextButton = domEl(`.pagination-${table} .next-btn`);
        let prevButton = domEl(`.pagination-${table} .prev-btn`);
        ...
        domEl(`.pagination-${table} .from`).innerText = begin;
        domEl(`.pagination-${table} .to`).innerText = end;
        domEl(`.pagination-${table} .page-number .page`).innerText = currentPage;
        ...
        let equalDivider = domEl(`.pagination-${table} .dots-3`);
        let firstDots = domEl(`.pagination-${table} .dots-1`);
        let lastDots = domEl(`.pagination-${table} .dots-${totalPages - 1}`);
        page = parseInt(page);

        const hideAllButtons = () => {
        domEls(`.pagination-${table} .btn`).forEach((el) => {
        hide(el, true);
        });
        }
        ...
        unhide(`.pagination-${table} .btn-${defaultsArray[x]}`);
        }
        }
        ...
        unhide(`.pagination-${table} .btn-${x}`);
        }
        unhide(`.pagination-${table} .btn-1`);
        unhide(`.pagination-${table} .btn-${totalPages}`);
        }
        } else {
        defaultMode();
        }
