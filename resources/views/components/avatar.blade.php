...
@aware([
    // these attributes could be passed from the x-avatars component also
    'dotted' => $dotted ?? false,
...
<div class="relative inline-block ltr:mr-2 rtl:ml-2 mt-1 rounded-full avatar {{ $image_size }} {{$stacked_css}} {{$class}} @if($showRing) ring-2 ring-offset-2 ring-offset-white ring-{{(!empty($bgColor) ? $bgColor : 'gray')}}-200 dark:ring-0 dark:ring-offset-dark-500/50  @endif">
