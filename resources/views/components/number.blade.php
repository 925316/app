...
<span class="inline-flex number-{{$name}} place">
    <x-input
...
<x-script :nonce="$nonce">
    changeCss('.number-{{$name}} .prefix svg', '!size-4,size-6,!stroke-2', 'remove');
    changeCss('.number-{{$name}} .suffix svg', '!size-4,size-6,!stroke-2', 'remove');
    domEl('.number-{{$name}} .suffix').addEventListener('click', () => {
    domEl('.number-{{$name}} input.{{$name}}').value = parseInt(domEl('.number-{{$name}} input.{{$name}}').value)
    + parseInt({{$step}});
    checkMinMax('{{$min}}', '{{$max}}', '{{$name}}', 1);
    });
    domEl('.number-{{$name}} .prefix').addEventListener('click', () => {
    domEl('.number-{{$name}} input.{{$name}}').value = parseInt(domEl('.number-{{$name}} input.{{$name}}').value)
    - parseInt({{$step}});
    checkMinMax('{{$min}}', '{{$max}}', '{{$name}}', 1);
    });
