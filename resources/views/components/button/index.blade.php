...
@php
    ...
    $base_button_css = ($circular) ? 'button-circle' : 'button '.(($uppercasing) ? 'uppercase ' : '');
...
<{{ $tag }} {{ $merged_attributes }} @if($disabled) disabled @endif @if($tag == 'button') type="{{ $button_type }}" @endif >
    @if(!empty($icon) && !$iconRight)
        <x-icon :name="$icon" :type="$iconType" class="stroke-2 {{$icon_css}}" />
    @endif
    @if (!$circular)
    ...
    @endif
    @if($hasSpinner)
        <x-spinner class="{{$icon_size[$size]}} !-mr-2 rtl:!-ml-2 !ml-2 rtl:!mr-2 {{ $spinner_css }}" />
    @endif
</{{$tag}}>