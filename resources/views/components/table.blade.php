...
@props([
    ...
    'searchPlaceholder' => config('bladewind.table.search_placeholder', __("bladewind.table_search_placeholder")),
    'searchField' => null,
    'searchDebounce' => 0,
    'searchMinLength' => 0,
    ...
    'noDataMessage' => config('bladewind.table.no_data_message', __("bladewind.table_no_data")),
    ...
    'defaultPage' => 1,
    'totalLabel' => config('bladewind.table.total_label', __("bladewind.pagination_label")),
    'limit' => null,
    'layout' => 'auto',
    'groupHeadingCss' => '',
...
<div @class([
    ...
    <div class="w-full">
        @if($searchable)
            <div class="table-filter-bar p-2 @if($hasShadow) drop-shadow shadow shadow-gray-200/70 dark:shadow-md dark:shadow-dark-950/20 @endif"
                            ...
                            <tr @class([
                                ...
                                @foreach($tableHeadings as $th)
                                    <td data-row-id="{{ $row_id }}"
                                        ...
                                        @if(!empty($onclick)) onclick="{!! build_click($onclick, $row) !!}" @endif>{!! $row[$th] !!}</td>
                                @endforeach
                                <x-table-icons :icons_array="$actionIcons" :row="$row"/>
                            </tr>
                        @endforeach
                    @endif
                    @else
...
@once
    <x-script :nonce="$nonce" :src="asset('resources/js/modules/table.js')"></x-script>
@endonce
@if($selectable)
    <x-script :nonce="$nonce">listenToSelectableTableRowEvents('{{$name}}');</x-script>
...
@if(!empty($searchContainer))
    <script>
        ...
        if (searchContainer) {
            searchContainer.innerHTML = '<div class="table-filter-bar-unhinged">' + domEl('div[data-table-name="{{$name}}"]').innerHTML + '</div>';
...
@if($checkable)
    @once
        <div class="hidden size-0 checkbox-template">
            <x-checkbox class="!size-5 !mr-0 rounded-md" label_css="mr-0" add_clearing="false"/>
        </div>
    ...
    @endonce
    <x-script :nonce="$nonce">
        addCheckboxesToTable('.table.{{$name}}');
        @if(!empty($selectedValue))
            checkSelected('.table.{{$name}}', '{{$selectedValue}}')
        @endif
    </x-script>
@endif
@if($sortable)
