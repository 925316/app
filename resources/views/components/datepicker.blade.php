...
@once
    <span class="datepicker-right-arrow hidden"><svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                        viewBox="0 0 24 24"
    ...
    <span class="datepicker-left-arrow hidden"><svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                       viewBox="0 0 24 24"
                                                       stroke-width="1.5" stroke="currentColor"
  ...
  <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 19.5 3 12m0 0 7.5-7.5M3 12h18"/>
    ...
    <x-script :nonce="$nonce">
        const MONTH_NAMES = {
        jan: '{{ __('bladewind.jan') }}',
        feb: '{{ __('bladewind.feb') }}',
        mar: '{{ __('bladewind.mar') }}',
        apr: '{{ __('bladewind.apr') }}',
        may: '{{ __('bladewind.may') }}',
        jun: '{{ __('bladewind.jun') }}',
        jul: '{{ __('bladewind.jul') }}',
        aug: '{{ __('bladewind.aug') }}',
        sep: '{{ __('bladewind.sep') }}',
        oct: '{{ __('bladewind.oct') }}',
        nov: '{{ __('bladewind.nov') }}',
        dec: '{{ __('bladewind.dec') }}',
        };
        const DAY_NAMES = {
        sun: '{{ __('bladewind.sun') }}'.substring(0, 3),
        mon: '{{ __('bladewind.mon') }}'.substring(0, 3),
        tue: '{{ __('bladewind.tue') }}'.substring(0, 3),
        wed: '{{ __('bladewind.wed') }}'.substring(0, 3),
        thu: '{{ __('bladewind.thu') }}'.substring(0, 3),
        fri: '{{ __('bladewind.fri') }}'.substring(0, 3),
        sat: '{{ __('bladewind.sat') }}'.substring(0, 3)
        };
    </x-script>
    <x-script :nonce="$nonce" src="{{ asset('resources/js/modules/datepicker.js') }}"></x-script>
@endonce
<x-script :nonce="$nonce">
    initCalendar({
    inputId: "{{$name}}",
