<aside x-data="{ open: true }" 
    ...
    <!-- Logo and Toggle -->
    <div class="relative flex items-center justify-between h-16 px-4 border-b border-cool-200/50 dark:border-cool-700/50">
        <!-- Logo Container -->
        <div class="flex items-center space-x-3" :class="{ 'opacity-0 w-0': !open, 'opacity-100 w-auto': open }">
        ...
        <button @click="open = !open"
            ...
            :class="{ 'right-3': open, 'right-4': !open }"
            aria-label="Toggle sidebar">
            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path :class="{ 'hidden': !open, 'inline-flex': open }" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 19l-7-7 7-7m8 14l-7-7 7-7" />
                <path :class="{ 'hidden': open, 'inline-flex': !open }" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 5l7 7-7 7M5 5l7 7-7 7" />
            </svg>
        </button>
    </div>
    ...
    <nav class="flex-1 overflow-y-auto py-4">
        <div class="space-y-1 px-2">
            ...
            @if (Auth::user()->hasPrivilege(7))
                <x-sidebar-nav-link :href="route('accounts.index')" :active="request()->routeIs('accounts.*')" :icon="'users'">
                    {{ __('Accounts') }}
                </x-sidebar-nav-link>
            @endif

            <!-- Licenses -->
            <x-sidebar-nav-link :href="route('licenses.index')" :active="request()->routeIs('licenses.*')" :icon="'key'">
                {{ __('Licenses') }}
            </x-sidebar-nav-link>

            <!-- Devices -->
            @if (Auth::user()->hasPrivilege(1))
                <x-sidebar-nav-link :href="route('devices.index')" :active="request()->routeIs('devices.*')" :icon="'device-mobile'">
                    {{ __('Devices') }}
                </x-sidebar-nav-link>
            @endif
            ...
            <!-- Packages -->
            @if (Auth::user()->hasPrivilege(1))
                <x-sidebar-nav-link :href="route('packages.index')" :active="request()->routeIs('packages.*')" :icon="'package'">
                    {{ __('Packages') }}
                </x-sidebar-nav-link>
            @endif
            ...
            <!-- Logs -->
            @if (Auth::user()->hasPrivilege(7))
                <x-sidebar-nav-link :href="route('logs.index')" :active="request()->routeIs('logs.*')" :icon="'document-text'">
                    {{ __('Logs') }}
                </x-sidebar-nav-link>
            @endif
        </div>
    ...
    <div class="border-t border-cool-200/50 dark:border-cool-700/50 p-4">
        ...
        <div class="flex items-center justify-between mb-4" :class="{ 'justify-center': !open }">
            <!-- User Avatar -->
            <div class="shrink-0">
                <div class="h-8 w-8 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-semibold text-sm"
                     :class="{ 'h-6 w-6 text-xs': !open }">
                    {{ strtoupper(substr(Auth::user()->username, 0, 1)) }}
                </div>
            </div>
            
            <!-- User Info - Hidden when collapsed -->
            <div class="flex-1 min-w-0 ml-3" x-show="open" x-transition>
            ...
            <div class="shrink-0 ml-3" x-show="open" x-transition>
                <x-dark-mode-toggle />
            </div>
        </div>
