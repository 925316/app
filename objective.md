...
## CRITICAL DESIGN CONSTRAINTS (AI MUST FOLLOW)
...
* **Web Context**: Users and administrators use the **EXACT SAME ROUTE URLs**. Permission differentiation happens **INSIDE CONTROLLERS**.
* **API Context**: Dedicated `routes/api.php` for Client/Software communication. API response MUST be JSON.

### 3. License State Machine

* State transitions MUST follow documented rules exactly (especially `revoked` as terminal state).
* Concurrency Control: 
  - Critical operations (Bind, Activate) MUST use Database Transactions.
  - To prevent race conditions, the `licenses` table MUST track the `current_device_id`. 
  - A Unique Constraint or logic check on `licenses.current_device_id` ensures one license = one device.

### 4. Device Binding Restrictions

* An account can have **ONLY ONE ACTIVE DEVICE** at a time.
* Current active device: `bound_at` IS NOT NULL AND `unbound_at` IS NULL.
* **HWID Logic**: HWID must be stored using **IRREVERSIBLE HASH** (SHA-256).

### 5. Client-Server Communication Protocol

* **Transport Security**: ALL API requests MUST use **HTTPS**.
* Replay Attack Protection: 
  - Requests MUST include a `nonce` (unique random string) and a `timestamp`.
  - Server MUST verify `nonce` has not been used within the last 5 minutes.
* **Response Signing (Anti-Tamper)**:
* The Server holds a **Private Key (RSA-2048 or Ed25519)**.
* Critical API responses (Check, Activate) MUST include a `signature` field.
* The `signature` is generated by signing the `data` JSON payload (including `nonce` and `timestamp`) with the Private Key.
* This allows the C++ Client (holding the Public Key) to verify the server is genuine (preventing "localhost" emulation attacks).
...
3. **Create API Routes**: Define endpoints in `api.php`.
   ...
   * Logic:
   ...
   - Check if `hwid` matches `account_devices`.
   - If HWID mismatch: Return error "Device Mismatch".
   - If valid: Construct Data JSON -> Sign with `CryptoService` -> Return JSON + Signature.
